...
def enqueue_dispatch_notification(
    *,
    session_factory: Callable[[], Session],
    recipient_id: int,
    notification_type: NotificationType,
    ...
    job_queue.enqueue(
        Job(
            name=f"dispatch-notification:{recipient_id}:{notification_type.value}",
            handler=create_notification,
            max_retries=settings.DISPATCH_NOTIFICATION_MAX_RETRIES,
        )
    )
